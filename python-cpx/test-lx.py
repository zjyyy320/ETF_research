...
# # 测试指数
# df_dax = web.DataReader("^DAX", "stooq", "2026-02-16", "2026-02-16")
# print("DAX on 2026-02-16:")
# print(df_dax)

# # 测试 ETF
# df_etf = web.DataReader("EXS1.DE", "stooq", "2026-02-16", "2026-02-16")
# print("\nEXS1.DE on 2026-02-16:")
# print(df_etf)
...
CONFIG = {
    "end_date": "2026-02-05",
    "long_path": r"./ETF跟踪指数量价数据-非日度更新/",
    "short_path": r"./ETF跟踪指数量价数据-日度更新/",
    "wind_params": {
...
EXTERNAL_SYMBOLS = {
    "^DJI": "2006-01-01",      # 道琼斯工业指数
    "^SPX": "2006-01-01",      # S&P 500 指数
    "^NKX": "2006-01-01",      # 日经225指数
    "1321.JP": "2006-01-01",   # 日经225 ETF (野村)
    "^DAX": "2006-01-01",      # 德国DAX指数
    "EXS1.DE": "2006-01-01"    # 德国DAX ETF (iShares)
}
...
def _fetch_clean_stooq(name: str, symbol: str, start: str, end: str) -> pd.DataFrame | None:
    """
    从 Stooq 获取并清洗海外指数/ETF 数据，格式对齐 Wind 数据
    """
    try:
        # 从 Stooq 下载（注意：Stooq 默认返回降序，最新在前）
    ...
    # 将 Stooq 的 DatetimeIndex 转为 'date' 列
    df = df.reset_index()
    df.rename(columns={df.columns[0]: "date"}, inplace=True)  # 第一列一定是日期
    ...
    if mask_null.any():
        ...
        # 删除 end 当天的空值行（防止未来数据污染）
        to_delete = mask_null & is_end_day
        if to_delete.any():
            deleted_dates = df[to_delete]["date"].dt.strftime("%Y-%m-%d").unique()
            #输出需要删除的行的具体值
            deleted_rows = df[to_delete]
            print(f"⚠️ {name} ({symbol}): 以下行含空值，即将删除：")
            print(deleted_rows[["date", "open", "close", "high", "low", "volume"]])
            df = df[~to_delete].copy()
            for d in deleted_dates:
                print(f"⚠️ {name} ({symbol}): {d} 含空值，已删除此行")

        # Stooq 默认降序 → 用 bfill 填充（即用“前一个交易日”填充）
        if not df.empty:
            was_null = df[price_cols + [vol_col]].isnull().any(axis=1)
            df[price_cols + [vol_col]] = df[price_cols + [vol_col]].fillna(method='bfill')
            now_valid = ~df[price_cols + [vol_col]].isnull().any(axis=1)
            ...
            if filled_mask.any():
                filled_dates = df[filled_mask]["date"].dt.strftime("%Y-%m-%d").unique()
                for d in filled_dates:
                    print(f"⚠️ {name} ({symbol}): {d} 含空值，已补充为前一日数据")
    ...
    print(f"✅ {name} ({symbol}): 抓取 {len(df)} 条有效数据")
    return df
# ===================== 主流程 =====================
...
def update_external_short_data(new_end: str = NEW_DATE):
    ...
    for symbol, short_df in new_data.items():
        ...
        combined.sort_values("date", ascending=False, inplace=True)
        combined["date"] = combined["date"].dt.strftime("%Y-%m-%d")
        _save_df(combined, symbol, CONFIG["short_path"])
